...
export default function ProductList() {
    ...
    const openEdit = (p) => {
        ...
        setEditStock(p.stock > 0 || p.stock === undefined || p.stock === null || p.stock === true);
        ...
        if (p.seller) {
            if (typeof p.seller === 'string') {
                if (/^[a-f\d]{24}$/i.test(p.seller)) {
                    ...
                    const match = stores.find(st => st.name.toLowerCase() === p.seller.toLowerCase());
                    if (match) storeId = match._id;
                }
            } else if (p.seller._id) {
                storeId = p.seller._id;
            } else if (p.seller.name) {
                const match = stores.find(st => st.name.toLowerCase() === p.seller.name.toLowerCase());
                if (match) storeId = match._id;
            }
        }
    ...
    const saveEdit = async (e) => {
        ...
        try {
            ...
            const { data } = await axios.patch(`/api/admin/products/${editProduct._id}`, {
                name: editName,
                price: Number(editPrice),
                description: editDescription,
                category: editCategory,
                image: imageUrl,
                images: editImagesStr.split(',').map(x => x.trim()).filter(Boolean),
            ...
            if (data.success && data.product) {
                setProducts((prev) =>
                    prev.map((x) => (String(x._id) === String(data.product._id) ? data.product : x))
                );
    ...
    const handleDelete = (p) => {
        ...
        axios
            ...
            .then(() => {
                setProducts((prev) => prev.filter((x) => String(x._id) !== String(p._id)));
                ...
                >
                    ...
                    >
                        <div className="form-card-header">
                            <h2 id="edit-product-title" className="form-card-title">Edit Product</h2>
                            <p className="form-card-sub">Update all product details, inventory, and category</p>
                        </div>
                        <form className="admin-form" onSubmit={saveEdit}>
                            
                            {/* Store Assignment */}
                            <div className="form-section-label" style={{ marginTop: '0px' }}>Store Assignment</div>
                            ...
                            <div className="form-row-2">
                                ...
                                <div className="form-group">
                                    <label className="form-label">Discount Price <span className="required">*</span></label>
                                    <div className="input-with-prefix">
                                        <span className="input-prefix">Rs.</span>
                                        <input
                                            className="form-input with-prefix"
                            ...
                            <div className="form-group">
                                <label className="form-label">Product Picture <span className="required">*</span></label>
                                <label className="upload-box product-img-upload" htmlFor="edit-product-img-upload" style={{ cursor: 'pointer' }}>
                                    {imagePreview || editImageUrl
                                        ? (
                                            <div className="product-img-preview-wrap">
                                                <img src={imagePreview || editImageUrl} alt="Product Preview" className="upload-preview product-img-preview" />
                            ...
                            <div className="form-group">
                                <label className="form-label">Extra Image URLs (comma-separated)</label>
                                <textarea
                                    className="form-input form-textarea"
                                    rows={2}
                            ...
                            <div className="form-row-2">
                                <div className="form-group">
                                    <label className="form-label">Max Order <span className="required">*</span></label>
                                    <input
                                        className="form-input"
                                        type="number"
                                        min="1"
                                ...
                                <div className="form-group stock-toggle-group" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                                    ...
                                    <label className="stock-toggle" style={{ cursor: 'pointer' }}>
                                        ...
                                        <span className={`toggle-label ${editStock ? 'in-stock' : 'out-stock'}`}>
                            ...
                            <tbody>
                                {products.map((p) => (
                                    <tr key={p._id} className="product-row">
                                        <td>
                                            {p.image_url || p.image
                                                ? <img src={p.image_url || p.image} alt={p.name} className="product-thumb" />
